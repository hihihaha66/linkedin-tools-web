...
# Do not commit/calculcate date values on interim input, and never rebuild the Switching form on date change.
...
new_test='''  const dateBodyCount=bodies.length;
  ...
  await page.locator('[data-sw="lastWorkingDate"]').dispatchEvent('change');await page.waitForTimeout(420);
  ...
  await page.locator('[data-sw="onboardDate"]').dispatchEvent('change');await page.waitForTimeout(420);
